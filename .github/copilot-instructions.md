...
- `uv run ruff check --fix` for auto-fixes
- `uv run ruff format` for formatting
- `uv run ruff check --select I --fix` for import sorting
- `uv run ruff check --select D` for docstring checks
...
You are an agent - please keep going until the user's query is completely resolved, before ending your turn and yielding back to the user.description: Beast Mode Dev
---
...
## Technology Stack
- **Backend**: Python 3.12+, SQLModel, Google ADK, LiteLLM, SQLite
- **CLI**: Typer + Rich for modern Python CLI with progress/colors
- **Video**: PyAV (FFmpeg) for metadata extraction
- **Architecture**: Hexagonal (ports/adapters), repository pattern, DDD
- **Testing**: pytest with TDD - contract tests (interfaces), integration tests (real SQLite), unit tests (fake UoW)
- **AI**: Google ADK + Gemini Flash 2.0, multi-agent validation system

## Critical Architecture Patterns

### Testing Strategy (75/75 tests passing)
- **Contract Tests**: Verify interface compliance (`tests/contracts/`)
- **Integration Tests**: Real SQLite database (`tests/integration/`)
- **Unit Tests**: Fake UoW for isolation (`tests/unit/`)
...
Each feature has comprehensive documentation in `specs/XXX-feature-name/`:
...
## MVP 0 Principle
"Essential functionality only, measure first, optimize later" - implement the minimum viable features with high quality, avoiding premature optimization or complex abstractions.
