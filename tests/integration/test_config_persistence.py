...
class TestConfigPersistence:
    ...
    def test_save_and_load_config_preserves_values(self, temp_config_path):
        ...
        custom_config = TimerConfig(
            work_duration_minutes=30, short_break_minutes=10, long_break_minutes=20, theme="dark"
        )
...
class TestConfigValidation:
    ...
    def test_config_rejects_zero_duration(self):
        ...
        with pytest.raises(ValueError, match="must be between"):
            TimerConfig(work_duration_minutes=0, short_break_minutes=5, long_break_minutes=15)

    def test_config_rejects_negative_duration(self):
        """Test that negative duration is rejected."""
        # Act & Assert
        with pytest.raises(ValueError, match="must be between"):
            TimerConfig(work_duration_minutes=-5, short_break_minutes=5, long_break_minutes=15)
    ...
    def test_config_accepts_valid_duration(self):
        ...
        # Act
        config = TimerConfig(
            work_duration_minutes=30, short_break_minutes=10, long_break_minutes=20
        )
